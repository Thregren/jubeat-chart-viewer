...
- **marker 顺序数字**：按「换气」分句编号 1、2、3…，**同一时刻一起出现（要一起按）的共用同一个编号**。
  分句规则：某个间隔**比周围稀疏**（≥ 最近 8 个间隔中位数的 1.25 倍，且至少 0.5 拍）就认为是一句结束、
  从 1 重新数；另外数到 9 也会重新数——这样数字**保证是个位**（密集谱面里光调断句阈值降不下来：
  间隔是量化的 0.5/0.75 拍，倍数 1.05~1.25 触发的其实是同一批断点，两位数占比都卡在 13%）。
