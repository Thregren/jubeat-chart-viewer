...
看着就像按钮坏了。现在控制条**上方会浮出一条进度**，按阶段显示：
...
- **进度是浮层（pop），不占布局**：早先它放在流里，出现/消失会把下面的东西顶来顶去，
  整页高度一直变；现在绝对定位挂在控制条上方，`pointer-events: none`，纯提示
- **音源没就绪时播放按钮是禁用的**（也会蓝掉一个「加载中」的外环）：
  以前允许点、点完排队到数据到齐自动起播，但那时候谱面状态可能还没落好，
  听起来就是「提前响了、不对拍」。现在直接禁掉；键盘（空格）走同一个判断，只会提示一句
