...
- **面板叠层显示**：可开关的**总连击**计数与 **marker 顺序数字**（同一秒内按出现先后编号 1、2、3…）
...
2. **按住**：marker 动画停住（冻结帧压到 15% 当底纹），该格改为从 12 点顺时针**逐渐填满的扇形 + 居中倒计时**（剩余秒数，≥10s 显示整数，否则一位小数）；跨格 hold 的头尾两格都显示
3. **末拍**：倒计时走完、扇形清空、格子闪一下收尾——**hold 不再播 marker 的尾部动画**
