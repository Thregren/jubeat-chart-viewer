...
- **marker 顺序数字**：同一秒内按出现先后编号 1、2、3…，**同一时刻一起出现（要一起按）的共用同一个编号**，
  这一批数字带**浓厚的霓虹光晕 + 两圈往外扩散的波纹**（同一批同步呼吸，一眼看出哪些是同时按的），
  **光晕严格裁在 marker 格子内、不会溢到相邻格**；数字跟 marker 一起出现、一起消失，数字本身和光晕各有一个开关
  （「marker 顺序数字」「同押光晕」），设置都会记住
- **同押光晕是双色**：光晕配色从 6 组预设里挑（青/洋红、琥珀/蓝、薄荷/珊瑚、柠檬/紫、天蓝/玫红、橙/青绿，
  都是一眼能分开的对比色），**不给自定义取色**——挑两个太接近的颜色等于没区分。
  同押挨得密的地方（相邻两批间隔 ≤ 0.35s），相邻两批**在主色和副色之间交替**，
  这样一眼能看出哪几个键是一起按的；稀疏的地方只有一批，只用主色，画面不会太花
