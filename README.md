...
`tail_tip` 本身是有用的：它就是游戏里长押那个**三角箭头指的方向**（这条长押往哪边收尾）。
现在照着画出来了，但**是静止的**：

- 位置、大小、朝向都固定，只在进入接近段时淡入、松手后消失
- 深色底 + 亮边，压在 hold 的浅色扇形上也能看清；和光晕 / 数字一样**裁在自己的格子里**
- 它和 marker 无关：marker 选「无（仅面板灯）」时箭头照常显示

> 试过让箭头「接近时滑进位、按住时往外推」的移动动画，实际看起来太闹（hold 那一格本来就有
> 扇形填充 + 倒计时），所以去掉了。
...
（`index.html` 本身会回源校验，所以它是即时生效的）。数字只要**和上一版不同**就行，
惯例是用当前版本号。
